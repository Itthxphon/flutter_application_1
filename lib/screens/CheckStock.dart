...
class CheckStockScreen extends StatefulWidget {
  ...
  const CheckStockScreen({super.key, this.scaffoldKey});
...
class _CheckStockScreenState extends State<CheckStockScreen> {
  ...
  void _navigateToCheckStock(String saleOrderNo) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => StockDetailScreen(orderNo: saleOrderNo),
      ),
    );
    ...
    if (result == true) {
      _fetchOrders(); // 🔁 โหลดข้อมูลใหม่ ถ้ามีการอัปเดตจากหน้า StockDetail
    }
  }
              ...
              IconButton(
                ...
                onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('คุณมีงานค้างที่ยังไม่จัดของ'),
                    ),
                  );
                },
              ),
                  ...
                  : RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return GestureDetector(
                          onTap: () => _navigateToCheckStock(orderNo),
                          child: Container(
